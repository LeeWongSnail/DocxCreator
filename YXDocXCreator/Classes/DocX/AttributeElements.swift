...
extension Dictionary where Key == NSAttributedString.Key{
    var runProperties:AEXMLElement{
        ...
        if let style=self[.underlineStyle] as? Int, let color=self[.foregroundColor] as? NSColor, let underline=NSUnderlineStyle(rawValue: style) {
        ...
        if let style=self[.strikethroughStyle] as? Int, let strikeThrough=NSUnderlineStyle(rawValue: style) {
